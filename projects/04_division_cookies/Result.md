...
void app_main(void)
{
    ...
    // ประกาศตัวแปรเก็บจำนวนคุกกี้
    int total_cookies = 30;        // คุกกี้ทั้งหมด
    int number_of_friends = 6;     // จำนวนเพื่อน
    int cookies_per_person;        // คุกกี้ต่อคน
    int remaining_cookies;         // คุกกี้ที่เหลือ
    ...
    vTaskDelay(3000 / portTICK_PERIOD_MS);\
    ESP_LOGI(TAG, "🔍 คุกกี้ %d ชิ้น หารลงตัวกับ:", total_cookies);
    for (int people = 1; people <= 10; people++) {
        if ( total_cookies % people == 0) {
            ESP_LOGI(TAG, "   ✅ %d คน → คนละ %d ชิ้น", 
                     people, total_cookies / people);
            }
        }       
...
```
<img width="1920" height="1020" alt="image" src="https://github.com/user-attachments/assets/44d30226-ce5b-418c-a9ed-70d67aa96a63" />
